...
- macOS clean-profile 自动 smoke 已有记录；手动 vault workflow 仍是 partial / blocked，不能写成 full pass。
...
- `v0.1.0-rc1` 实际 release：本轮只准备文档和 checklist，不创建 tag 或 GitHub Release；跟踪 issue：[#212](https://github.com/Aidenwu0209/llm-wiki-desktop/issues/212)。
- macOS manual vault smoke：自动 smoke 和 app launch 已有记录，临时 vault 创建 / 导入 / plan / dashboard 仍需人工复验；跟踪 issue：[#213](https://github.com/Aidenwu0209/llm-wiki-desktop/issues/213)。
...
- `v0.1.0-rc1` unsigned release candidate：准备 release notes 和 publish checklist；实际 tag / GitHub Release 由 [#212](https://github.com/Aidenwu0209/llm-wiki-desktop/issues/212) 跟踪。
...
本轮目标是把仓库整理到可评审的 unsigned release candidate 准备状态：
...
- 更新 macOS smoke 文档，明确自动 smoke 通过记录、`build:app` 记录和 manual vault workflow partial / blocked 状态。
