...
- 提供基础 Graph 入口：展示 source -> claim -> concept / review / proposal / warning 的证据关系，并补充 Obsidian `[[wikilink]]`、frontmatter `sources:` / `source_path:` 共享来源关系、共享邻居推荐、同类型页面加权和桥接节点 insight，帮助定位 traceability break、阅读路径和 insight 写回位置。
...
- `Graph` 当前是 evidence navigation graph，优先可追踪性、断点定位、共享邻居阅读推荐、同类型页面加权和桥接节点 insight，不追求复杂社区发现或布局算法。
