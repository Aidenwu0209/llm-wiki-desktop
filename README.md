...
桌面端会按 SHA-256 跳过重复资料。PDF / 图片默认优先使用 PaddleOCR-VL-1.5；未启用 OCR Parser、未配置 endpoint 或所选 API key 环境变量不可见时，ingest plan 会明确阻塞且不会上传 raw document。用户也可以显式切回 `auto/local-text` 本地 fallback。
...
- 对 PDF / 图片输入默认走 `paddleocr-vl15` 计划状态；配置齐全后由桌面端把所选 API key 环境变量作为子进程环境变量传给 runtime `pdf_to_markdown.py --parser layout-api --api-url <PaddleOCR endpoint>`。未配置时不上传 raw document；用户显式切回 `auto/local-text` 才走本地 selectable-text fallback。
