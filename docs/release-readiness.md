...
| Release candidate | `npm ci && npm run build && npm test && npm run build:app && npm run smoke:macos:bundle` | Builds the frontend, runs Rust tests, creates local app bundles, and verifies the packaged macOS app opens a visible window. |
...
1. Run `npm run smoke:macos:bundle` and confirm the packaged app starts with at least one visible window.
2. Launch `src-tauri/target/release/bundle/macos/LLM Wiki.app`.
3. Confirm startup restores the last selected vault or shows the centered Welcome page with New Project / Open Project / recent / demo actions.
4. Create a project and confirm project name, template, AI output language, and parent directory are persisted in desktop settings.
5. Open a generated DeepSeek vault, not the outer workspace or raw PDF folder.
6. Confirm Dashboard shows vault health, runtime, Obsidian, lint/review/writeback status, and next actions.
7. Open Raw Sources and confirm Refresh / Import / Folder, source list, preview, details drawer, and Obsidian actions work.
8. Run a small runtime action and confirm Activity shows status, duration, command, logs, cancel/timeout/retry controls, and persisted history.
9. Open Traceability and confirm warning cards can open claim/source/artifact targets or expose fallback paths.
10. Open Chat / Search and confirm vault search results include source/claim/concept/review/proposal relations and an evidence map.
11. Convert a grounded chat question into a Query Writeback proposal and confirm no concept/source page is modified before approval.
12. Approve and apply only a safe test proposal, then confirm post-apply dashboard refresh and lint behavior.
13. Open Graph and confirm source -> claim -> concept/review/proposal/warning relations are usable for evidence navigation.
14. Open Settings / LLM Models and About, confirm provider toggles, local CLI check, logo, version, repo link, and runtime boundary are visible.
...
Latest recorded runs: the 2026-05-28 CST scripted macOS smoke passed on commit `e3005080248ee7ad920a9ac4d874d42aeedcd511`; the 2026-05-31 CST release DMG manual vault smoke passed the temporary vault create/import/plan/Dashboard/Raw Sources/Wiki Chat workflow with a non-blocking standalone cache warning fixed after the RC. See [`artifacts/smoke/macos/smoke-macos-clean-profile-20260528-summary.md`](../artifacts/smoke/macos/smoke-macos-clean-profile-20260528-summary.md) and [`docs/reports/smoke-macos-release-manual-vault-20260531-summary.md`](reports/smoke-macos-release-manual-vault-20260531-summary.md).
...
- `bash scripts/smoke/macos-bundle-launch.sh --app <mounted-or-copied-app> --require-signature` against the exact public release asset.
