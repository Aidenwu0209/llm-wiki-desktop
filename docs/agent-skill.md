...
When the gate passes, the API may expose only localhost, token-protected, read-only routes:
...
Until a live server exists, ask the desktop app for the readiness report first. If it is not enabled, the agent should stay on the existing UI-backed flow:
